...
# Fixed: Saved sqlite:///sacco.db locally in project folder instead of temporary /tmp/ directory
...
[data-theme="dark"] { --bg: #1a202c; --card: #2d3748; --text: #f7fafc; --subtext: #a0aec0; --border: #4a5568; --nav-bg: #0f172a; --nav-admin: #0f172a; --nav-admin: #0f172a; --input-bg: #4a5568; }
...
.btn { button { padding: 8px 15px; background: #2b6cb0; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; }
...
.avatar-placeholder { width: 45px; height: 45px; border-radius: 50%; background: #abac0; color: white; display: flex; align-items: center; justify-content: center; font-weight: bold; }
...
<div class="card" style="width: 360px;">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2>SACCO Portal</h2>
        <button class="btn-theme" onclick="toggleTheme()" style="color:var(--text);">Border</button>
    </div>
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <h2>Emergency Fund Transfer Requests (Friend Help)</h2>
            <table>
                <thead>
        ...
        <div class="card">
            <h2>Registered Members & Breakdown</h2>
            <table>
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><style color="#38a169">KES {{ "{:,.2f}".format(m.weekly_savings) }}</style></td>
                        <td><style color="#2b6cb0">KES {{ "{:,.2f}".format(m.monthly_savings) }}</style></td>
                        <td><style color="#dd6b20" font-weight: bold;>KES {{ "{:,.2f}".format(m.emergency_savings) }}</style></td>
                        <td>
        ...
        <div class="card">
            <h2>Deposit Verification Requests</h2>
            <table>
                <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ p.id }}</td>
                        <td>{{ p.member_name }} <small>({{ p.username }})</small></td>
        ...
        <div class="card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ r.id }}</td>
                        <td>{{ r.member_name }} <small>({{ r.username }})</small></td>
                        <td>{{ r.phone }}</td>
                        <td>{{ r.date }}</td>
    ...
    <div class="container">
            ...
            <form method="POST" action="/member/notify_payment">
                <div class="form-grid">
                    ...
                    <div>
                        ...
                        <input type="text" name="ref_code" placeholder="e.g. QCH8092XS" required>
                    </div>
                </div>
                <br>
                ...
                <div class="form-grid">
                    <div>
                        ...
                        <select name="recipient_username" required>
                            ...
                            <option value="{{ o.username }}">{{ o.name }} (@{{ o.username }})</option>
                            {% endfor %}
                        </select>
                    </div>
                    <div>
                        <label>Amount (Max KES {{ "{:,.2f}".format(member.emergency_savings) }})</label>
                        <input type="number" name="amount" max="{{ member.emergency_savings }}" required>
                    </div>
                </div>
                <br>
        ...
        <div class="card">
            ...
            <div style="max-height: 200px; overflow-y: auto;">
                <table>
                    <thead>
                        <tr><th>Week</th><th>Th Paid Amount</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        {% for w in weeks %}
                        <tr>
        ...
        <div class="card">
            ...
            <div style="max-height: 200px; overflow-y: auto;">
                <table>
                    <thead>
                        <tr><th>Month</th><th>Th Paid Amount</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        {% for m in months %}
                        <tr>
